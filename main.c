...
uint32_t erase_block_sector(uint32_t address) {
  ...
  if (takeDownTest) {
    ...
    if (takeDownPeriod != 0) {
      ...
      uint32_t failLen = rand() % NORFAT_SECTOR_SIZE;
      uint8_t failRand = rand() % 4;
      if (failLen == 0 || failLen == NORFAT_SECTOR_SIZE || failRand == 0) {
        uint8_t failVal = rand() % 0xFE;
        memset(&block[address], failVal, NORFAT_SECTOR_SIZE);
      } else {
        memset(&block[address], 0xFF, failLen);
      }
...
int PowerFailOnWriteTest(norFAT_FS *fs) {
  ...
  res = norfat_mount(fs);
  res = norfat_fopen(fs, "validate.bin", "w", &f);
...
int PowerStressTest(norFAT_FS *fs) {
  ...
  res = norfat_fclose(fs, &f);
#if 1
  res = norfat_fopen(fs, "powercycles.txt", "w", &f);
  ...
  while (res == 0) {
    ...
    if (res == NORFAT_OK) {
      ...
      res = norfat_fopen(fs, "powercycles.txt", "wb", &f);
      if (res == NORFAT_OK) {
        res = norfat_fwrite(fs, &powerCycleTest, 1, 4, &f);
        res = norfat_fclose(fs, &f);
      }
    ...
    if (res == NORFAT_ERR_FILE_NOT_FOUND) {
      ...
      if (memcmp(test, compare, testLength)) {
        ...
        printf("Test file did not match at %i 0x%X 0x%X 0x%X 0x%X\r\n", i,
               test[i], compare[i], (uint32_t)&test[i], (uint32_t)&compare[i]);
        break;
      }
      res = norfat_fclose(fs, &f);
    }
...
int deleteTest(norFAT_FS *fs) {
  ...
  res = norfat_format(fs);
  res = norfat_mount(fs);
  res = norfat_fopen(fs, "testfile.bin", "wb", &f);
  if (res == NORFAT_ERR_FILE_NOT_FOUND) {
    return 1;
  }
...
int main(int argv, char **argc) {
  ...
  norFAT_FS fs1 = {.addressStart = 0,
                   .tableCount = NORFAT_TABLE_COUNT +
                                 4, // FAT tables carved out of flash sectors
